import { Col, Image, Rate, Row, Select } from "antd";
...
const ProductDetailsComponent = ({ idProduct }) => {
  ...
  const [errorSize, setErrorSize] = useState(false); // State cho lỗi chọn size
  ...
  const handleAddOrderProduct = () => {
    if (!user?.id) {
      ...
      setErrorSize(true); // Hiển thị lỗi nếu chưa chọn size
    } else {
      const orderRedux = order?.orderItems?.find(
        (item) => item.product === productDetails?._id
      );
        ...
        dispatch(
          addOrderProduct({
            orderItem: {
              ...
              size: selectedSize, // Gửi size vào action Redux
  ...
  const handleSelectSize = (size) => {
    ...
    setErrorSize(false); // Ẩn lỗi nếu chọn size
  };
      ...
      >
        ...
        <Col span={14} style={{ paddingLeft: "10px" }}>
          ...
          >
              ...
              {productDetails?.size?.map((size) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "4px",
                    cursor: "pointer",
                    background: size === selectedSize ? "#f0f0ff" : "#fff",
                  }}
                >
                  {size}
                </button>
            ...
            <div style={{ margin: "20px 0" }}>Số lượng</div>
            <WrapperQualityProduct>
              <button
                style={{
                  border: "none",
